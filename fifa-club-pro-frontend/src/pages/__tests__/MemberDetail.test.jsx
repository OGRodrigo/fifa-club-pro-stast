...
function mockDetailRequests({ members = makeMembers(), matches = makeMatches() } = {}) {
...
describe("MemberDetail", () => {
  ...
  it("carga el detalle del miembro y sus partidos del club", async () => {
    ...
    // Rodri sí debe aparecer
    expect(screen.getAllByText(/Rodri/i).length).toBeGreaterThan(0);

    // La pantalla muestra acciones para abrir partidos del historial reciente
    expect(
  ...
  it("calcula stats usando solo partidos del club actual", async () => {
    ...
    // m1 + m2 para u1 = 2 partidos, 3 goles, 1 asistencia, avg rating 8.75
    // m3 no pertenece al club-1 y no debe contaminar stats.
  ...
  it("permite navegar al detalle de un partido", async () => {
    ...
    expect(mockNavigate.mock.calls.some(([arg]) => String(arg).includes("/matches/"))).toBe(true);
  });
