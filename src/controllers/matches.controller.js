...
exports.createMatch = async (req, res) => {
  try {
    ...
    for (const ps of playerStats) {
      ...
      const maxGoalsForClub = psClub === homeId ? Number(scoreHome) : Number(scoreAway);
...
exports.getMatches = async (req, res) => {
  try {
    ...
    if (isNaN(pageNum) || pageNum < 1) return res.status(400).json({ message: "page inválida" });
 ...
 * (No toca playerStats; eso se puede hacer con un PATCH separado si quieres)
 * PUT /matches/:id
 * =====================================================
 */
exports.updateMatch = async (req, res) => {
  try {
    ...
    if (homeClub !== undefined) match.homeClub = homeClub;
    if (awayClub !== undefined) match.awayClub = awayClub;
...
exports.getMatchMVP = async (req, res) => {
  try {
    ...
    const map = new Map(); // userId -> { goals, assists, points, clubId }

    match.playerStats.forEach(ps => {
      const uid = ps.user._id.toString();
      const uname = ps.user.username;
...
/**
 * =====================================================
 * PATCH PLAYER STATS (NO rompe updateMatch)
 * -----------------------------------------------------
 * PATCH /matches/:id/player-stats
 ...
 * - sum(assists) por club <= score del club (realista)
 * =====================================================
 */
exports.updateMatchPlayerStats = async (req, res) => {
  try {
    ...
    // acumuladores por club para validar totales
    let homeGoalsSum = 0, awayGoalsSum = 0;
    let homeAssistsSum = 0, awayAssistsSum = 0;
    ...
    for (const ps of playerStats) {
      ...
      // coherencia por jugador vs score del club
      const maxGoalsForClub = psClub === homeId ? Number(match.scoreHome) : Number(match.scoreAway);
      if (g > maxGoalsForClub) {
        return res.status(400).json({ message: "Goles del jugador exceden los goles del club" });
      }
      if (g + a > maxGoalsForClub) {
        return res.status(400).json({ message: "Goles + asistencias exceden los goles del club" });
      }
    ...
    // ✅ Validaciones “reales” por equipo
    // assists totales no pueden superar goles del equipo (máx 1 asistencia por gol)
    ...
    // ✅ opcional estricto: suma de goles de jugadores = score del equipo
    if (strictTotals) {
    ...
    // devolver match full (útil para frontend)
    const populated = await Match.findById(match._id)
      .populate("homeClub", "name country")
      .populate("awayClub", "name country")
      .populate("playerStats.user", "username gamerTag platform");
